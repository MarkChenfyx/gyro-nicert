...
class BacktestingEngine:
    ...
    def __init__(self) -> None:
        ...
        self.logs: list = []
        self.bar_data_loader: Optional[Callable] = None
    ...
    def set_data_loaders(
        ...
        tick_loader: Optional[Callable] = None,
    ) -> None:
        """注入工作台行情加载器，避免依赖 vn.py 全局数据库配置。"""
        self.bar_data_loader = bar_loader
        self.tick_data_loader = tick_loader
    ...
    def _load_bars(
        ...
        loader = self.bar_data_loader or load_bar_data
        return loader(symbol, exchange, interval, start, end)
    ...
    def _load_ticks(
        ...
        loader = self.tick_data_loader or load_tick_data
        return loader(symbol, exchange, start, end)
    ...
    def load_data(self) -> None:
        ...
        while start < self.end:
            ...
            if self.mode == BacktestingMode.BAR:
                data: List[BarData] = self._load_bars(
                    self.symbol,
                    self.exchange,
                    self.interval,
                    start,
                ...
                data: List[TickData] = self._load_ticks(
                    self.symbol,
                    self.exchange,
                    start,
                    end
    ...
    def run_backtesting(self) -> None:
        ...
        for ix, i in enumerate(range(0, total_size, batch_size)):
            ...
            completed = min(i + len(batch_data), total_size)
            progress = completed / total_size if total_size else 1
            progress_bar: str = "=" * max(1, int(progress * 10))
    ...
    def show_chart(self, df: DataFrame = None ) -> None:
        ...
        fig = make_subplots(
            rows=4,
            cols=1,
            subplot_titles=[self.vt_symbol+'_surge', "Drawdown", "Daily Pnl", "Pnl Distribution"],
        ...
        fig.show()
        # fig.write_image('.\债券ETF回测结果\\'+self.vt_symbol+'.png')
    ...
    def load_bar(
        ...
        bars: List[BarData] = self._load_bars(
            symbol,
            exchange,
            interval,
            init_start,
    ...
    def load_tick(self, vt_symbol: str, days: int, callback: Callable) -> List[TickData]:
        ...
        ticks: List[TickData] = self._load_ticks(
            symbol,
            exchange,
            init_start,
            init_end
...
def wrap_evaluate(engine: BacktestingEngine, target_name: str) -> callable:
    """
    Wrap evaluate function with given setting from backend.backtesting engine.
    """
