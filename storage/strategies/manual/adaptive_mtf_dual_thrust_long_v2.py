...
class AdaptiveMtfDualThrustLongV2Strategy(CtaTemplate):
    """Long-only Dual Thrust with adaptive 30m/60m entry permission."""
    ...
    emergency_stop = 0.0
    structure_low = 0.0
    previous_structure_pivot_low = 0.0
    structure_armed = 0
    ...
    def __init__(
        ...
        self._trend_30m_closes: Deque[float] = deque(
            maxlen=max(int(self.trend_ma_bars), 2)
        )
        self._trend_60m_closes: Deque[float] = deque(
            maxlen=max(int(self.trend_ma_bars), 2)
        )
    ...
    def _make_aggregate_bar(
        ...
        return BarData(
            ...
            # Aggregated signals become available only when the final minute closes.
    ...
    def _update_aggregate(
        ...
        # Only completed bars may update multi-timeframe state.  In particular,
        # the 59-minute morning residue is not treated as a 60-minute bar.
        if not complete:
            return None
    ...
    def _update_30m_state(self, bar: BarData) -> bool:
        ...
        trend_window = max(int(self.trend_ma_bars), 2)
        ...
        if is_pivot and pivot_after_entry and rebound:
            prior_pivot_low = self.previous_structure_pivot_low
            self.previous_structure_pivot_low = pivot_low

            # A single local low is not a confirmed higher low.  Arm the
            # structural stop only after a later pivot is above its predecessor,
            # and never loosen an already armed support level.
            confirmed_higher_low = (
                prior_pivot_low > 0
                and pivot_low > prior_pivot_low
            )
            tighter_support = (
                not self.structure_armed
                or pivot_low > self.structure_low
            )
            if confirmed_higher_low and tighter_support:
    ...
    def _reset_position_state(self) -> None:
        ...
        self.previous_structure_pivot_low = 0.0
