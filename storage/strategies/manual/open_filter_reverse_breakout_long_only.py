...
class OpenFilterReverseBreakoutLongOnlyStrategy(CtaTemplate):
    """开盘方向过滤、反向收盘确认和移动止损组合策略（只做多）。"""
    ...
    def on_init(self) -> None:
        self.write_log("只做多开盘过滤反向突破策略初始化")
        self.load_bar(self.lookback_window + 10)

    def on_start(self) -> None:
        self.write_log("只做多开盘过滤反向突破策略启动")
        self.put_event()

    def on_stop(self) -> None:
        self.write_log("只做多开盘过滤反向突破策略停止")
        self.put_event()
    ...
    def on_bar(self, bar: BarData) -> None:
        ...
        if self.pos == 0:
            self._handle_flat(bar, long_breakout)
        elif self.pos > 0:
    ...
    def _handle_flat(self, bar: BarData, long_breakout: bool) -> None:
        self._reset_trade_state()
        allow_long = not self.use_open_filter or bar.close_price > self.day_open_price
        if long_breakout and allow_long:
