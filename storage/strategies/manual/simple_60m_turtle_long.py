...
class Simple60mTurtleLongStrategy(CtaTemplate):
    """Unit-position, long-only Turtle strategy on completed 60-minute bars."""
    ...
    def on_init(self) -> None:
        self.write_log("简单60分钟海龟策略初始化")
        self.load_bar(max(self.entry_window, self.atr_window) + 20)

    def on_start(self) -> None:
        self.write_log("简单60分钟海龟策略启动")
        self.put_event()

    def on_stop(self) -> None:
        self.write_log("简单60分钟海龟策略停止")
        self.put_event()
