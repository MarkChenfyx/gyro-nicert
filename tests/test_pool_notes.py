...
def _prepare_source_run(root: Path) -> None:
    run_path = root / "run_1"
    variant_path = run_path / "variants" / "baseline"
    ...
    (variant_path / "result.json").write_text(json.dumps({"metrics": {"sharpe": 1.0}}), encoding="utf-8")
